...
def match_type_8(input_string):
    all =re.findall(r'\s\d\d[- /.]\w\w\w[- /.]\d\d\d\d\s', input_string)
    for s in all:
        s=s.strip()
        print("A"+s+"a")
        x='-'
...
def match_type_8a(input_string):
    all =re.findall(r'\s\d\d[- /.]\w\w\w[- /.]\d\d\s', input_string)
    for s in all:
        s=s.strip()
        x='-'
        for a in s:
...
def match_type_9(input_string):
    all =re.findall(r'\s\w\w\w[- /.]\d\d[-]\d\d\s', input_string)
    for s in all:
        s=s.strip()
        x='-'
        for a in s:
...
def match_type_10(input_string):
    all =re.findall(r'\s\w\w\w[- /.]\d\d[- /.]\d\d\d\d\s', input_string)
    for s in all:
        s=s.strip()
        x='-'
        for a in s:
